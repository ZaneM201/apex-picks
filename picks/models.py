...
class RacePicks(models.Model):
    ...
    pole_position = models.ForeignKey(Driver, on_delete=models.CASCADE, related_name='pole_position_picks')
    fastest_lap = models.ForeignKey(Driver, on_delete=models.CASCADE, related_name='fastest_lap_picks')
    driver_of_the_day = models.ForeignKey(Driver, on_delete=models.CASCADE, related_name='driver_of_the_day_picks')

    # Sprint Race Picks (if applicable)
    ...
    def clean(self):
        """Validate picks are different drivers"""
        ...
        if self.race.sprint_race:
            ...
            if None not in sprint_picks and len(set(sprint_picks)) != 3:
                raise ValidationError("Sprint Podum positions must be different drivers!")

class RaceResult(models.Model):
    """Actual results of race for scoring"""
    ...
    pole_position = models.ForeignKey(Driver, on_delete=models.CASCADE, related_name='pole_position_results')
    fastest_lap = models.ForeignKey(Driver, on_delete=models.CASCADE, related_name='fastest_lap_results')
    driver_of_the_day = models.ForeignKey(Driver, on_delete=models.CASCADE, related_name='driver_of_the_day_results')

    # DNF Penalties
    dnf_drivers = models.ManyToManyField(Driver, on_delete=models.CASCADE, related_name='dnf_results', null=True, blank=True)
    ...
    def point_calculatation(self, race_picks):
        ...
        if race_pick.driver_of_the_day == self.driver_of_the_day:
            points += 20
